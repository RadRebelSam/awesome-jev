...
const lastCoverage = coverageLog.runs?.at(-1) ?? null;
  ...
  const body = [
    ...
    '---',
    '',
    `<sub>${entries.length} entries · ${exclusive} of them in none of the ${lastCoverage?.sourcesChecked ?? 0} other Jev directories checked on ${lastCoverage?.on ?? 'never'} · last updated ${today()} · ${stats.review ?? 0} candidates in the [review queue](data/review-queue.md) · generated by [\`tools/fetch.js\`](tools/fetch.js)</sub>`,
  ].join('\n');
